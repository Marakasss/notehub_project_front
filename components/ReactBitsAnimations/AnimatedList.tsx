...
const AnimatedList: React.FC<AnimatedListProps> = ({
  ...
  useEffect(() => {
    ...
    // avoid synchronous setState inside effect: schedule reset after paint and cancel if unmounted
    const rafId = requestAnimationFrame(() => setKeyboardNav(false));
  ...
  return (
    <div className={`relative w-full ${className}`}>
      <div
        ref={listRef}
        className={`h-[calc(100vh-130px)] overflow-y-auto p-4 ${
          displayScrollbar
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div
              className={`p-4 border border-cyan-900 w-full rounded-lg ${
                selectedIndex === index ? "bg-[#01232f]" : ""
              } ${itemClassName}`}
            >
              {item}
            </div>
